      ...
      pages: [
        {
          ...
          description: "Receptionist's main dashboard",
          path: "/receptionist",
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">
            Complete overview of all system pages organized by roles
          </p>
        </div>
      </div>
      ...
      {Object.entries(pagesByRole).map(([role, roleData], roleIndex) => (
        <div key={role} className="space-y-4">
          <div className="flex justify-between space-x-3 border-b pb-3">
            <div
              className={`flex items-center space-x-2 px-3 py-2 rounded-lg border ${roleData.color}`}
            >
              {roleData.icon}
              <h2 className="text-xl font-semibold">{role} Access</h2>
            </div>
            <Badge variant="outline" className="text-sm">
              {roleData.pages.length} page
              {roleData.pages.length !== 1 ? "s" : ""}
            </Badge>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pl-4">
            {roleData.pages.map((page, pageIndex) => (
              <Card
                key={pageIndex}
                className="hover:shadow-lg transition-all duration-200 border-l-4 border-l-gray-300 hover:border-l-blue-500"
              >
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge className={getBadgeTypeColor(page.badge)}>
                      {page.badge}
                    </Badge>
                  </div>
