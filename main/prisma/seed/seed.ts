...
async function main() {
  ...
  const createdCounterCategories = await Promise.all(
    counterCategories.map(async (cat) => {
      const department = createdDepartments.find(d => d.name === 'Cardiology'); // Assigning to a default department for now
  ...
  const createdUsers = await Promise.all(
    users.map(async (user) => {
      ...
      const department = createdDepartments.find(d => d.name === user.department);
